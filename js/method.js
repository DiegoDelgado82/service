...
async function getMarcs() {
  let url =
    "https://raw.githubusercontent.com/DiegoDelgado82/serviciotecnico/main/st.json";
...
function removeOptions(ddl) {
  var j,
    L = document.getElementById("ddlCategoria").options.length - 1;
  for (j = L; j >= 0; j--) {
    ddl.remove(j);
  }
}
...
async function renderBuscar() {
  ...
  users.forEach((user) => {
    if (
      user.MARCA === document.getElementById("ddlMarca").value &&
      user.CATEGORIA === document.getElementById("ddlCategoria").value
    ) {
      ...
      if (user.WEB != "-") {
        ...
        if (user.CORREO != "-") {
          ...
          if (user.TELEFONO != "-") {
            texto =
              "Dirección Web: " + user.WEB + " Teléfono: " + user.TELEFONO;
            if (user.TELEFONO2 != "-") {
              texto =
                "Dirección Web: " +
                user.WEB +
            ...
            if (user.TELEFONO2 != "-") {
              texto =
                "Dirección Web: " + user.WEB + " Teléfono: " + user.TELEFONO2;
            }
          }
        }
      }
